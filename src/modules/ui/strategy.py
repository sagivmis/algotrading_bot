...
# from apis. import BitmexClient

if typing.TYPE_CHECKING:
    from ui.root import Root


class StrategyPanel(tk.Frame):
    def __init__(self, root: "Root", binance: BinanceFuturesClient, *args, **kwargs):
        ...
        self._base_params = [
            {
                ...
                "data_type": str,
                "values": ["Technical", "Breakout", "VWAP_Breakout"],
                "width": 10,
            },
            {
                "code_name": "contract",
        ...
        self._extra_params = {
            ...
            "VWAP_Breakout": [
                {
                    "code_name": "days_before_vwap",
                    "name": "Days Before - VWAP",
                    "widget": tk.Entry,
                    "data_type": float,
                    "value": 6,
                },
                {
                    "code_name": "days_before_ema",
                    "name": "Days Before - EMA",
                    "widget": tk.Entry,
                    "data_type": float,
                    "value": 6,
                },
    ...
    def _switch_strategy(self, b_index: int):
        ...
        if self.body_widgets["activation"][b_index].cget("text") == "OFF":
            ...
            if len(new_strategy.candles) == 0:
                self.root.logging_frame.add_log(
                    f"No historical data retreived for {contract.symbol}"
                )
                return

            if exchange == "Binance":
                self._exchanges[exchange].subscribe([contract], "aggTrade")
