...
class Watchlist(tk.Frame):
    def __init__(
        ...
        bybit_contracts: typing.Dict[str, Contract],
        *args,
        **kwargs,
    ):
        ...
        self._bybit_symbols = list(bybit_contracts.keys())
        ...
        self._bybit_label.grid(row=0, column=1)
        self._bybit_entry = tk.Entry(
            self._commands_frame,
            fg=FG_COLOR,
            justify=tk.CENTER,
            insertbackground=FG_COLOR,
        ...
        self._bybit_entry.bind("<Return>", self._add_bybit_symbol)

        self._bybit_entry.grid(row=1, column=1)
    ...
    def _add_bybit_symbol(self, event):
        symbol = event.widget.get()
        if symbol in self._bybit_symbols:
            self._add_symbol(symbol, "Bybit")
            event.widget.delete(0, tk.END)
